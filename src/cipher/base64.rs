...
impl<R: Read> traits::EncodeBase64Stream for R {
    fn encode_base64_stream<W: Write>(&mut self, writer: &mut W) -> traits::Result<()> {
        ...
        std::io::copy(self, &mut encoder)
            .map_err(|reason| traits::Error::Base64StreamEncode(reason.to_string()))?;
...
impl<R: Read> traits::DecodeBase64Stream for R {
    fn decode_base64_stream<W: Write>(&mut self, writer: &mut W) -> traits::Result<()> {
        ...
        std::io::copy(&mut decoder, writer)
            .map_err(|reason| traits::Error::Base64StreamDecode(reason.to_string()))?;
...
pub mod tests {
    ...
    use crate::{DecodeBase64, DecodeBase64Stream, EncodeBase64, EncodeBase64Stream};
